...
// datalayer function
window.dataLayerPush = function(name) {
  // dataLayer.push({
  //   "event": name
  // });
  // show/hide log for all datalayer push'es
  console.log("Pushed to GTM:" + '"' + name + '"');
}
...
FlyoutBox.prototype.setupEvents = function(){
    ...
    var prevPos = window.pageYOffset || document.documentElement.scrollTop,
        ...
        showHideOnScroll = function() {
            ...
            content.offsetHeight * 0.2 + content.offsetTop < nextPos + window.outerHeight * 0.5 ? this.showMe() : this.hideMe();
            prevPos = nextPos;
        }.bind(this);
    window.addEventListener('scroll', function ( event ) {
      console.log("content.offsetHeight * 0.2", "content.offsetTop", "nextPos", "window.outerHeight * 0.5");
      console.log(content.offsetHeight * 0.2, content.offsetTop, nextPos, window.outerHeight * 0.5);
      if(!isFirstScrolled){
